...
TsIRTUtils::TsIRTUtils() {;}


TsIRTUtils::~TsIRTUtils() {;}


G4double TsIRTUtils::erfcx_y100(G4double y100)
...
G4double TsIRTUtils::NormQuantile(G4double p)
{
...
G4double TsIRTUtils::erfcx(G4double x)
{
    if (x >= 0) {
...
G4double TsIRTUtils::erfc(G4double x) {
    return 1.0 - std::erf(x);
}


G4double TsIRTUtils::erfcWxy(G4double c, G4double x, G4double y) {
    return c * ( erfc(x) - std::exp(-x*x) * erfcx(x+y) );
}


G4int TsIRTUtils::FindBin(G4double x, std::vector<G4double> vx) {
    G4int bin;
    if ( x < vx[0] ) // underflow
        return -1; //vx[0];
    if ( x > vx[ vx.size()-1] ) //overflow
        return -1; // vx[ vx.size()-1];
    
    for ( bin = 0; x > vx[bin]; bin++);
    return bin;
}


G4int TsIRTUtils::FindBin(G4int n, G4double xmin, G4double xmax, G4double value) {
    G4int bin = -1;
    if ( value <= xmin )
        bin = 0; //1;
...
std::vector<G4double> TsIRTUtils::CreateTimeSteps(G4double t0, G4double tf, G4int tbins, G4bool isLog) {
    G4double logTMin = std::log10(t0);
    G4double logTMax = std::log10(tf);
    G4double Dt = (logTMax-logTMin) / (tbins - 1);
    G4double ti = 1.0*ps;
    std::vector<G4double> theTimes;
    if ( isLog ) {
        for ( int i = 0; i < tbins; i++ ) {
            ti = std::pow(10, logTMin + i * Dt);
            theTimes.push_back(ti);
        }
    } else {
        while ( ti < 1e-6*s ) {
            if ( ti < 10.0 * ps )
                ti += 0.1 * ps;
            else if ( ti >= 10.0*ps && ti < 100.0*ps)
                ti += 1.0 * ps;
            else if ( ti >= 100.0*ps && ti < 1000.0*ps)
                ti += 3.0 * ps;
            else if ( ti >= 1000.0*ps && ti < 10000.0*ps)
                ti += 10.0 * ps;
            else if ( ti >= 10000.0*ps)
                ti += 100.0 * ps;
            theTimes.push_back(ti);
        }
    }
...
G4double TsIRTUtils::SampleTypeII(G4double alpha, G4double sigma, G4double r0, G4double D) {
    G4double beta = (r0 - sigma)/2;
    G4double alphatilde = -alpha;
    G4double betaOverAlphaTilde = beta/alphatilde;
    G4double p = 2.0 * std::sqrt(2.0*betaOverAlphaTilde);
    G4double q = 2.0 / std::sqrt(2.0*betaOverAlphaTilde);
    G4double M = 1.0/(alphatilde*alphatilde);
    if ( M < 3.0*betaOverAlphaTilde )
        M = 3.0*betaOverAlphaTilde;
    ...
    if ( W < Winf ) {
        ...
        while(1) {
            ...
            u = G4UniformRand();
            lambdax = Lambda(X, beta, alphatilde);
            if ((X <= 2.0*betaOverAlphaTilde && u <= lambdax) ||
                (X >= 2.0*betaOverAlphaTilde && u*M/X <= lambdax))
                break;
            ntrials++;
            if ( ntrials > 10000 )
                return -1.0;
...
G4double TsIRTUtils::erfcInv(G4double x) {
