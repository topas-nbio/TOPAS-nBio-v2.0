...
// Basophils have a 2-3 lobed nucleus and contain many granules.

#include "TsBasophil.hh"
...
TsBasophil::TsBasophil(TsParameterManager* pM, TsExtensionManager* eM, TsMaterialManager* mM, TsGeometryManager* gM,
...
TsBasophil::~TsBasophil()
{;}

void TsBasophil::ResolveParameters() {
    BasophilRadius  = fPm->GetDoubleParameter(GetFullParmName("BasophilRadius"), "Length");
...
G4VPhysicalVolume* TsBasophil::Construct()
{
	BeginConstruction();
    ...
    G4Orb* gBasophil = new G4Orb(fName, BasophilRadius);
    fEnvelopeLog = CreateLogicalVolume(gBasophil);
    fEnvelopePhys = CreatePhysicalVolume(fEnvelopeLog);
    
    
    //***********************************************************************
    // Nucleus is generally bilobed.
    // Nucleus is formed from the union of two spheres.
    //***********************************************************************
    
    G4String subComponentName1 = "Nucleus";
    G4double Sphere1Radius = 2.0*um;
    G4double Sphere2Radius = 3.0*um;
    ...
    G4UnionSolid* gNucleus = new G4UnionSolid(subComponentName1, solidSphere1, solidSphere2, nuclRotation, G4ThreeVector(3*um,2*um,0));
