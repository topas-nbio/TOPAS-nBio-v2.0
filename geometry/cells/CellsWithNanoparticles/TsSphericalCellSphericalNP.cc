...
G4VPhysicalVolume* TsSphericalCellSphericalNP::Construct()
{
    ...
    if (fPm->ParameterExists(nameMembrane)) {
        ...
        CreatePhysicalVolume("Membrane", lMembrane, rotationMatrix, CellPosition, fEnvelopePhys);
    ...
    if (fPm->ParameterExists(nameNPNuc)) {
        ...
        for (int m = 0; m < NbOfNP; m++){
            ...
            CreatePhysicalVolume("Nanoparticle", m, true, lNP, rotationMatrix, AddNanoparticleAtSphereSurface(rNP, 0), fEnvelopePhys);
    ...
    if (fPm->ParameterExists(name6)) {
        ...
        for (int k = 0; k < MitoNumber; k++){
           ...
           CreatePhysicalVolume("Mitochondria", k, true, lMito, rotationMatrix, AddSphereToCell(MitoRadius), fEnvelopePhys);
        ...
        if (fPm->ParameterExists(name8)) {
            ...
            for (int m = 0; m < NbOfNP; m++){
                ...
                CreatePhysicalVolume("Nanoparticle", m, true, lNP, rotationMatrix, AddNanoparticleAtSphereSurface(rNP, NPatMitochondria), fEnvelopePhys);
    ...
    if (fPm->ParameterExists(name9)) {
        ...
        for (int m = 0; m < NbOfNP; m++){
            ...
            CreatePhysicalVolume("Nanoparticle", m, true, lNP, rotationMatrix, AddSphereToCell(rNP), fEnvelopePhys);
        }
    }
